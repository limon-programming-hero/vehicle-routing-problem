...
const veOperatingCost = (numOfVehicle) => {
    const vehicleOperatingCostF1 = 200; //  taking vehicle operating cost = 5000 tk per vehicle;
...
const cargoDamageCost = (demand, route_distance, remainingLoad) => {
    ...
    const deteriorationDuringTransportationK1 = 1;
    const deteriorationDuringUnloadingK2 = 1;
...
const freshnessCost = (demand, single_route_distance) => {
    const costPerUnitTimeF4 = 10; //taking as a constant cost per unit time;
    const unloadingCostPerUnitTimeF5 = 15; //taking as a constant value;
...
const energyCost = (remainingLoad, fuelPriceF6, demand, single_route_distance) => {
    ...
    const fullLoadFuelConsumptionU1 = 0.102; // fuel consumption for full load vehicle (Liter per unit distance))
...
const totalCO2EmissionCost = (fuelConsumptionFC) => {
    ...
    const CO2UnitPriceF7 = .25; //the unit price of carbon dioxide gas emission
    const CO2QuotaTq = 100; //carbon emission quota
